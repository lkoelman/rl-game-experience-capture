...
FrameTimestampLogger::FrameTimestampLogger(const std::string& filepath) : out_csv_(filepath, std::ios::out | std::ios::trunc) {
    if (!out_csv_.is_open()) {
...
FrameTimestampLogger::~FrameTimestampLogger() = default;

void FrameTimestampLogger::LogFrame(std::uint64_t monotonic_ns, std::uint64_t pts) {
