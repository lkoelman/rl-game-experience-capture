...
├── include/
│   ├── RecordingSession.hpp             # Orchestrator
│   ├── VideoRecorder.hpp       # GStreamer pipeline wrapper
│   ├── GamepadLogger.hpp         # SDL3 event loop and serializer
│   ├── FrameTimestampLogger.hpp          # Fast CSV logger
│   └── Replayer.hpp            # Offline dataset converter
├── src/
│   ├── RecordingSession.cpp
│   ├── VideoRecorder.cpp
│   ├── GamepadLogger.cpp
│   ├── FrameTimestampLogger.cpp
│   ├── Replayer.cpp
...
#### A. Sync Logger (`include/FrameTimestampLogger.hpp`)
...
class VideoRecorder {
public:
    VideoRecorder(const std::string& output_path, std::shared_ptr<FrameTimestampLogger> sync_logger);
    ~VideoRecorder();
...
#### C. Input Logger (`include/GamepadLogger.hpp`)
...
class GamepadLogger {
public:
    explicit GamepadLogger(const std::string& output_path);
    ~GamepadLogger();
...
#### D. The Orchestrator (`include/RecordingSession.hpp`)
...
public:
    RecordingSession(const std::string& output_dir, const std::string& session_name);
    void Start();
    void Stop();
...
In C++, graceful shutdown requires handling OS-level signals. On Windows, `std::signal(SIGINT)` is insufficient for console applications; the agent must use `SetConsoleCtrlHandler` or cross-platform equivalent wrappers to trigger `RecordingSession::Stop()`.
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        RecordingSession session(output_dir, session_name);
        session.Start();
