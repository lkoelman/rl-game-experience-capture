...
The PoC has validated our solution. This has been tested to work while running a video game. The next step is to modify the InputLogger used by the `record_session` executable to use the same gamepad forwarding strategy. We should read and save the physical gamepad using SDL3 (already implemented), but emit virtual inputs using ViGEmBus. Read the implementation of the virtual gamepad bridge and use a similar approach.