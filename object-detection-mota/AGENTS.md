...
- When running python code, always use the virtual environment `.venv` managed by `uv`
  - use `uv run python3` instead of `python3`
  - alternatively, first activate the virtual environment, and then run regular python commands

- After writing new python code, run the ruff linter and fix any errors, then run formatter
  - run `ruff check . --fix`
...
- Module-specific README files (e.g., `mimic_dex/module/**/README.md`)

**When to update docs:**
- New features: add how-to guide or update relevant README
- API changes: update module README and architecture docs
- Build/test changes: update this CLAUDE.md or module-specific docs
